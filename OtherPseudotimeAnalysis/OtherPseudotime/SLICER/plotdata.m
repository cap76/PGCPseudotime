...
l2 = importdata('slicer_pseudotime_PGC1.txt');
l3 = importdata('slicer_pseudotime_soma1.txt');
%l2 = importdata('./Pseudotime_pgconly.csv')
%l3 = importdata('./Pseudotime_somaonly.csv')
%l3 = importdata('Pseudotime_somaonly.csv');
...
for i = 1:length(l2.data)
    inds2(i,1) = find(strcmp(upper(l2.textdata{i+1,2}), upper(D1.textdata(1,2:end)))==1);
end

for i = 1:length(l3.data)
    inds3(i,1) = find(strcmp(upper(l3.textdata{i+1,2}), upper(D1.textdata(1,2:end)))==1);
end
...
%uT = unique(D1.data(3,:));%unique(Output.Data.orig.origt)
%for i = 1:length(T2)
%    Ind2(i,1) = find( D1.data(3,i) == uT);
%end

%uT2 = unique(D1.data(3,:));%unique(Output.Data.orig.origt)
%for i = 1:length(T2)
%    Ind2(i,1) = find( D1.data(3,i) == uT2);
%end
...
vs = violinplot(T1(inds)',Ind(inds));

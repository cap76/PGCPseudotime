...
HSMM_sample_sheet <- read.delim("/Users/christopher_penfold/Desktop/BranchingGPs/demos/Botrytis_PhenoData2.txt", header=TRUE)
HSMM_expr_matrix <- read.table("/Users/christopher_penfold/Desktop/BranchingGPs/demos/Botrytis_data_2.txt",row.names = 1, header=TRUE)
...
write.table(as.data.frame(meh), "~/Desktop/Bot_infect_Monocle_allprocesses.txt", sep="\t")


#Read in the data
HSMM_gene_annotation <- read.delim("/Users/christopher_penfold/Desktop/BranchingGPs/demos/Botrytis_featureData.txt",row.names = 1)
HSMM_sample_sheet <- read.delim("/Users/christopher_penfold/Desktop/BranchingGPs/demos/Botrytis_PhenoData1.txt", header=TRUE)
HSMM_expr_matrix <- read.table("/Users/christopher_penfold/Desktop/BranchingGPs/demos/Botrytis_data_1.txt",row.names = 1, header=TRUE)
...
meh<-print((pData(HSMM)))
write.table(as.data.frame(meh), "~/Desktop/Bot_control_Monocle_allprocesses.txt", sep="\t")
...
SOX17_id <- row.names(subset(fData(HSMM), gene_short_name == "CATMA1a02710"))
WT1_id <- row.names(subset(fData(HSMM), gene_short_name == "CATMA2a12440"))
#DPPA3_id <- row.names(subset(fData(HSMM), gene_short_name == "DPPA3"))
cth <- addCellType(cth, "PGC", classify_func = function(x) { x[SOX17_id,] >= 11})
cth <- addCellType(cth, "Soma", classify_func = function(x) { x[WT1_id,] >= 10.5})
#cth <- addCellType(cth, "ES", classify_func = function(x) { x[SOX17_id,] < .4})
...
GM_state <- function(cds){
  if (length(unique(pData(cds)$State)) > 1){
    T0_counts <- table(pData(cds)$State, pData(cds)$Time)[,"0"]
...
meh<-print((pData(HSMM)))
write.table(as.data.frame(meh), "~/Desktop/Monocle_allprocesses.txt", sep="\t")

function K = covBranchingProcess_2C(hyp, x, z, i)

% Branching process covariance function. Two-component branching processes 
% where both branches diverge from a latent process (Matern-based).

%Should fully function with gpml.
...
if nargin<4                                                        % covariances  
    ...
    K(ind2,ind2) = K(ind2,ind2) + K3;            
        ...
        if isempty(ind4)==0  & isempty(ind2)==0          
...
elseif i==4
    ...
    if xeqz==0 & dg==0
    K = zeros(size(x,1),size(z,1));     
    if isempty(ind2)==0 & isempty(ind4)==0,K(ind2,ind4)   = feval(covN1{:}, [location,steepness2,hyp(6),hyp(7)], x(ind2,1),z(ind4,1),4);end        
    else  
    K = zeros(size(x,1),size(x,1));
  ...
  elseif i==8
     ...
     if xeqz==0 & dg==0
     K = feval(k1{:},[hyp(8),hyp(9)],x(:,1),z(:,1),1);
     else
     K = feval(k1{:},[hyp(8),hyp(9)],x(:,1),x(:,1),1);
     if dg,K = diag(K);end
     end

     elseif i==9
     if xeqz==0 & dg==0
     K = feval(k1{:},[hyp(8),hyp(9)],x(:,1),z(:,1),2);
     else
     K = feval(k1{:},[hyp(8),hyp(9)],x(:,1),x(:,1),2);
     if dg,K = diag(K);end
     end

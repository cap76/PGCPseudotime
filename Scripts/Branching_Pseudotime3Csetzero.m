function Output = Branching_Pseudotime3Csetzero(seed,missingT);

%Add path to various toolboxes (gpml, netlab and changepoint kernels)
addpath(genpath('../'))
%addpath(genpath('../Pseudotime_Algorithms/Functions/Others'))
%addpath(genpath('../Pseudotime_Algorithms/Functions/gpml-matlab-v3.6-2015-07-07/'))
%addpath(genpath('../Code/gpss-research/source/gpml/cov/'))
%addpath(genpath('../Code/gpss-research/source/matlab/custom-cov/'))
%addpath(genpath('../Code/deepGP/netlab3_3/'))

%Example covariance functions covBranching_v5a, covBranching_v5b,
%covBranching_v4b. 2 branch and 3 branch processes.

try

	sdladklsand load(['./Marker_Pseudotime_' num2str(seed) '_' num2str(missingT) '_3Crext.mat']) 
        %sdadasa        load(['~/Desktop/BranchingGPs/results/primordial_germ_cells/Pseudotime/Marker_Pseudotime_' num2str(seed) '_' num2str(missingT) '_2E_final2.mat'])            
...
rng(seed)
NoMCMC = 30000; %No. steps in MCMC

%Grab some data ...
[D1,t1,Type,Sex,uID,FixLabel,FixTime,Assign,nocells] = initialisedata(missingT);
%Initialise the prior distributions for all the hyperaprams
[options, prior, altprior] = initialiseprior(D1); %Initialise prior distributions
%Initialise the modes (assign unlabelled data on the fly)


[Data,X,Y,Xstar] = initialiseBranch(D1.data(4:end,:),t1,Type,Sex,uID,FixLabel,FixTime,Assign,prior);
%keyboard
%Tune switch points by fix other hyperparameters from here on out.
...
for arc = 1:size(Data.update.y,2)              %Inference method
    ind = find((Data.update.y(:,arc))~=pi);    %In each GP model ignore missing data ~=0 (change to pi when want to include everything).
    par = {'meanConst','covBranchingProcess_3Rec3','likGauss',Data.update.x(ind,:),Data.update.y(ind,arc)};
...
%Param.Store = cell(NoMCMC,1);
Param.Store{1} = Data; %Store everything at each step. Can now vectorise now we've sorted out a bunch of covariance functions.
   ...
   % else        
    ...
    if double(int64(i/100))==(i/100)
     ...
     %Every 100 steps do HMC on hyperparams. Maybe break this down ... the branch times are what we want to update really. 
         ...
         ind = find((Data.update.y(:,arc)~=pi));              
         par = {'meanConst','covBranchingProcess_3Rec3','likGauss',Data.update.x(ind,:),Data.update.y(ind,arc),Data.update.hyp{arc}};
         ...
         if double(int64(i/10))==(i/10)
            ...
            save(['./Marker_Pseudotime_' num2str(seed) '_' num2str(missingT) '_3Csetzero.mat'],'Output')                        
...
if length(Set2)>=2 %Update time for branch 2
    ...
    for j = 1:size(Y,2)        
        ind = find(Y(:,j)~=pi);
        par = {'meanConst','covBranchingProcess_3Rec3','likGauss', X(ind,:),Y(ind,j)};
...
Y = Data.update.y;
    ...
    for j = 1:size(Y,2)     
        ...
        ind = find(Y(:,j)~=pi);
        par = {'meanConst','covBranchingProcess_3Rec3','likGauss', X(ind,:),Y(ind,j),X(:,:)};
...
for arc = 1:size(Y,2)        
        ...
        ind = find(Y(:,arc)~=pi);
        par = {'meanConst','covBranchingProcess_3Rec3','likGauss', X(ind,:),Y(ind,arc)};
...
if Data.update.x(u,2)==1 %Current update is branch 1
    ...
    for j = 1:size(Y2,2)        
        ...
        ind = find(Y2(:,j)~=pi);
        par = {'meanConst','covBranchingProcess_3Rec3','likGauss', X2(ind,:),Y2(ind,j)};
        [L2(1,j) dL] = gp(Data.update.hyp{j}, Data.update.im, par{:}); 
    end    
...
elseif Data.update.x(u,2)==2 %Current locus is Branch 2
    ...
    for j = 1:size(Y1,2)
        ...
        ind = find(Y1(:,j)~=pi);
        par = {'meanConst','covBranchingProcess_3Rec3','likGauss', X1(ind,:),Y1(ind,j)};
...
for arc = 1:size(y1,1)
    ...
    hyp.lik  = 1;
    hyp.cov  = [ 0.45; 4; 0.45; 4; 0.6; 4; 0.5; 1; 0.5; 1; 0.5; 1];
    ...
    hyp_init = feval(@minimize, hyp, @gp, -50000, im, par{:});    
...
for cells = find(FixLabel==0) %These have no times or labels ...
    ...
    %Prediction points. Assigne to branch 1 or two only
    if y1(arc,cells)==0 %Missing data point, uniform distribution
        ...
        lp = log(ones(2*100,1)./2*100);        
        Lik = [Lik;lp'];
    else
        
        Xtest = [linspace(0,max(t1),100)',ones(100,1); linspace(0,max(t1),100)',2*ones(100,1)];
        ...
        im  = {@infPrior,@infExact,prior};
        par = {'meanConst','covBranchingProcess_3Rec3','likGauss', Xtrain, Ytrain, Xtest, Ytest};    
    ...
    Tim = [linspace(0,max(t1),100),linspace(0,max(t1),100)];%[unique(t1(find(t1>=0))),unique(t1(find(t1>=0)))];
    Labs = [ones(1,length(t1)),2*ones(1,length(t1))];
...
Data.orig.cov = 'covBranchingProcess_3Rec3';
...
prior.cov  = {p3;p4;p3;p4;p3;p4;p5;p6;p5;p6;p5;p6};     %Covariance hyp.


p1_alt = {@priorClamped};             %Mean
altprior.mean = {p1_alt};
altprior.lik  = {p1_alt};
altprior.cov  = {p3;p4;p3;p4;p3;p4;p1_alt;p1_alt;p1_alt;p1_alt;p1_alt;p1_alt};
...
%indsubset = randperm(size(D1.data,1));
%D1.data = D1.data(indsubset(1:double(int64(length(indsubset)/2))),:); #Randomly select a subset of locations
...
%Randomly set 20% of all genes to 0 
for i = 4:size(D1.data,1)
ind1 = 4:size(D1.data,1);
inds = randperm(length(4:size(D1.data,1)));
ind1 = ind1(inds);
D1.data(i,ind1(1:double(int64(0.2*(size(D1.data,2)))))) = 0;
end
...
FixLabel(1,:) = ones;     %i.e., do not update the label in the Gibbs step
...
Assign  = Type;
Assign(find(Type==2))=1; %Soma
Assign(find(Type==1))=2; %PGC
Assign(find(Type==-1))=3; %ESC
Assign(find(Type==0)) = randi([1 2],1,length(find(Type==0))); %Random assignment of pre-implantation and ESCs

%Assign(find(Assign==1))=3;
%Assign(find(Assign==2))=1;
%Assign(find(Assign==3))=2;
...
FixLabel(find(t1==6.1 | t1==missingT)) = 0; %We will update blastocyst cells

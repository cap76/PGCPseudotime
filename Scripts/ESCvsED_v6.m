function Output = ESCvsED_v6(batchi);
...
load(['Marker_Pseudotime_1_800_3B.mat'])
...
for i = startind:endind
    ...
    par1c = {'meanConst',k1,'likGauss',X(find(X(:,2)==1),1),Y(find(X(:,2)==1)),X(find(X(:,2)==1),1)}; %Predict at soma    
    par1d = {'meanConst',k1,'likGauss',X(find(X(:,2)==1),1),Y(find(X(:,2)==1)),X(find(X(:,2)==2),1)}; %Predict at PGC locations        
    ...
    clear prior hyp
    hyp.cov = [0.4; 4; log(3);log(3)]; hyp.mean = []; hyp.lik = log(2);
    prior.cov  = {cT2;pT;pLS;[]}; prior.lik = {pN};
    im = {@infPrior,@infExact,prior}; 
    par1a = {'meanZero',k2,'likGauss',X(find(X(:,2)==2),1),Y(find(X(:,2)==2))-ymu1d};
    par1b = {'meanZero',k2,'likGauss',X(find(X(:,2)==2),1),Y(find(X(:,2)==2))-ymu1d,Xstar(:,1)};    
    ...
    prior.mean = {[]}; prior.cov  = {pLS;[]}; prior.lik  = {[]};
    im = {@infPrior,@infExact,prior}; 
    par1a = {'meanConst',k1,'likGauss',X(find(X(:,2)==2),1),Y(find(X(:,2)==2))};
    par1b = {'meanConst',k1,'likGauss',X(find(X(:,2)==2),1),Y(find(X(:,2)==2)),Xstar(:,1)};    
    par1c = {'meanConst',k1,'likGauss',X(find(X(:,2)==2),1),Y(find(X(:,2)==2)),X(find(X(:,2)==2),1)}; %Predict at PGC    
    par1d = {'meanConst',k1,'likGauss',X(find(X(:,2)==2),1),Y(find(X(:,2)==2)),X(find(X(:,2)==3),1)}; %Predict at ESC locations        
    ...
    clear prior hyp
    hyp.cov = [0.6;4;0.4;4;log(3);log(3)]; hyp.mean = []; hyp.lik = log(2);
    prior.cov  = {cT3;pT;cT1;pT;pLS;[]}; prior.lik = {pN};
   
    im = {@infPrior,@infExact,prior}; 
    par1a = {'meanZero',k3,'likGauss',X(find(X(:,2)==3),1),Y(find(X(:,2)==3))-ymu1g};
    par1b = {'meanZero',k3,'likGauss',X(find(X(:,2)==3),1),Y(find(X(:,2)==3))-ymu1g,Xstar(:,1)};
    ...
    im         = {@infPrior,@infExact,prior};     
    par1a      = {'meanConst',k4,'likGauss',X,Y};
    par1b      = {'meanConst',k4,'likGauss',X,Y,Xstar};
    ...
    im       = {@infPrior,@infExact,prior};     
    par1a    = {'meanConst',k4,'likGauss',X1,Y};
    par1b    = {'meanConst',k4,'likGauss',X1,Y,Xstar};
    ...
    X3 = [x1,1*ones(size(x1,1),1); x2,1*ones(size(x2,1),1); x3,3*ones(size(x3,1),1)]; %Only soma different    
    hyp.cov  = [P_somapgc; P_pgcesc; hyp_pN2.cov(3:4); hyp_pN3.cov(5:6); hyp_pN1.cov(1:2)];    
    ...
    im       = {@infPrior,@infExact,prior};     
    par1a    = {'meanConst',k4,'likGauss',X3,Y};
    par1b    = {'meanConst',k4,'likGauss',X3,Y,Xstar};
    ...
    im         = {@infPrior,@infExact,prior};     
    par1a      = {'meanConst','covBranchingProcess_3Rec3','likGauss',X6,Y};
    par1b      = {'meanConst','covBranchingProcess_3Rec3','likGauss',X6,Y,Xstar};
    ...
    BIC = -2*L + [4,6,4,8,14,8,14,4]*log(length(Y));
    
    ESCvED{i}.gene = genes{i+2};
    ESCvED{i}.L    = L;
    ESCvED{i}.BIC  = BIC;    
    ...
    save(['ESCvED_6_' num2str(batchi) '_Extended.mat'],'ESCvED')    
    disp(['Step ' num2str(i)])
    catch
        ESCvED{i} = []; 
    end

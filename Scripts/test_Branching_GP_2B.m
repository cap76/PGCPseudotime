...
Y = y(1,101:300)' + rand(length(y(1,101:300)),1)*1;

%Prediction
Xstar = [linspace(0,10,1000)',ones(1000,1);linspace(0,10,1000)',2*ones(1000,1)];

%Initialise hyperparameters
hyp.cov = [8;0.5;0.5;rand(6,1)];%[3,0.5,0.5,1,3,0.3,2,2,2]';
hyp.mean = mean(X(:,1));
hyp.lik = .1;

%Optimise hyperparameters
hyp_pN      = feval(@minimize, hyp, @gp, -10000, @infExact, 'meanConst','covBranchingProcess_2B','likGauss',X,Y);

%Predictions
[ymu ys2 fmu fs2   ] = gp(hyp_pN, 'infExact', 'meanConst', 'covBranchingProcess_2B', 'likGauss', X, Y, Xstar);

%Now plot
K = covBranchingProcess_2B(hyp_pN.cov, X);

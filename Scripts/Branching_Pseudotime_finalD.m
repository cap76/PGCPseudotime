function Output = Branching_Pseudotime_finalD(seed,missingT);

%Add path to various toolboxes (gpml, netlab and changepoint kernels)
addpath(genpath('../'))

try %Try loading in previous run
    sdasd  load(['Marker_Pseudotime_' num2str(seed) '_' num2str(missingT) '_cov2D.mat'])
    ...
    if double(int64(i/100))==(i/100)
     ...
     %Every 100 steps do HMC on hyperparams. Maybe break this down ... the branch times are what we want to update really. 
         ...
         if double(int64(i/10))==(i/10)
             ...
             save(['Marker_Pseudotime_' num2str(seed) '_' num2str(missingT) '_cov2D_prD.mat'],'Output')            
...
for cells =  find(FixLabel==0) %These have no times or labels ...
    ...
    if y1(arc,cells)==0 %Missing data point, uniform distribution
        ...
        im  = {@infPrior,@infExact,prior};
        par = {'meanConst','covBranchingProcess_2D','likGauss', Xtrain, Ytrain, gridX, Ytest};    
...
%Okay now lets develop amnesia over these branch labels (blastocyst).
Type(find(t1==6 | t1 == missingT))     = -1; %NaN
FixLabel(find(t1==6 | t1 == missingT)) = 0;  %NaN
t1(find(t1==6 | t1 == missingT))       = -1; %NaN
